                ...
                {items.map((card, cardIndex) => (
                    ...
                    >
                        ...
                        <div className="flex items-center">
                            ...
                            <input
                                type="text"
                                placeholder="Item/Buff Name"
                                value={card.name}
                                onChange={(e) => updateCardName(card.id, e.target.value)}
                                onFocus={() => setFocusedInput({ cardId: card.id })}
                                onBlur={() => setFocusedInput(null)}
                                className={`text-sm text-white border border-gray-500 rounded-xs p-1 px-3 flex-grow focus:outline-none focus:ring-1 focus:ring-blue-400 ${!card.enabled ? 'opacity-50' : ''
                                    }`}
                            />
                            <button
